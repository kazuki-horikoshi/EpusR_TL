...
calc_humidity <- function(clo, Tair, velocity, SET_star) {
  ...
  # 湿度を逆算する
  rh_values <- seq(0, 100, by = 1)  # 湿度の範囲を設定
  best_rh <- NA
  min_diff <- Inf
  
  for (rh in rh_values) {
    # calcSETを使用して予測されたSETを計算
...
# comfort_dataをCSVファイルとして保存
write.csv(comfort_data, file = "localdir/AsimEx/Comfort_models/comfort_data_with_RH.csv", row.names = FALSE)
